...
let handleClickAvatar = async (userId) => {
...
let handleMemberJoined = async (MemberId) => {
  ...
  document.getElementById(`${MemberId}`).addEventListener("click", function () {
    handleClickAvatar(userRtcUid);
  });
};
...
const toggleMic = async (event) => {
  const e = event?.target || document.getElementById("mic-icon");
  if (micMuted) {
    e.src = "icons/mic.svg";
    e.style.backgroundColor = "ivory";
    micMuted = false;
  } else {
    e.src = "icons/mic-off.svg";
...
const micMute = async () => {
  const e = document.getElementById("mic-icon");
  e.src = "icons/mic-off.svg";
  e.style.backgroundColor = "indianred";
...
document
  .getElementById("mic-mute-all")
  .addEventListener("click", handleMuteAll);
